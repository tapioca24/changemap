...
残作業はBacklogへ移管済み（2026-09-23）。以後の進捗と受け入れ条件は以下のタスクを正本とする。

- [TASK-13：フェーズ0〜5のPRをレビューし依存順にマージする](<../backlog/tasks/task-13 - フェーズ0〜5のPRをレビューし依存順にマージする.md>)（完了。移管時の未完了扱いを訂正し、PR #1〜#6のマージと統合コミットのCI成功を確認済み）
- [TASK-14：初版のバージョンを設定しnpmへ公開する](<../backlog/tasks/task-14 - 初版のバージョンを設定しnpmへ公開する.md>)（TASK-13に依存。npm公開・バージョン更新は別途ユーザー指示が必要）

以下の各フェーズ追記は当時の記録として保持する。「次はフェーズ〜」などの記載は現在の残作業を示さない。
